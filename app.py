...
def generate_summary(place):
    prompt = (
        f"[공간 설명 요청]\n\n"
        f"당신은 감성적이고 창의적인 한국어 공간 디자이너입니다.\n"
        f"장소 이름: {place['name']}\n"
        f"주소: {place['formatted_address']}\n\n"
        f"이 장소는 어떤 분위기인지, 공간의 색감, 구조, 주변 환경, 문화적 맥락 등을 고려해서 서술해 주세요.\n"
        f"감각적이고 문학적인 표현을 사용하되, 너무 과장되지 않게 실재감 있게 묘사해 주세요.\n"
        f"공간을 처음 방문하는 사람도 생생하게 느낄 수 있도록, 감성적 설명 + 사실적 정보가 조화를 이루도록 작성해 주세요.\n"
    )
